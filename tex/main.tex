...
% In \mpc, we use syntax that matches the literature, which is
% sometimes different than what we are using in PAL\@. In particular,
% $\eproj{1}{e}$ is written \texttt{fst} $e$ (for ``first'') and
% $\eproj{2}{e}$ is written \texttt{snd} $e$ in PAL; $\einj{1}{e}$ is
% written \texttt{L} $e$ and $\einj{2}{e}$ is written \texttt{R} $e$;
% $\elam{x}{e}$ is written as \texttt{fun x -> }$e$ in PAL. The syntax for \texttt{case} (for
% unpackaging sum types) is slightly different also. We discuss syntax
% and derived forms a bit more in \Cref{sec:derived}.
