...
with MPC may want to read Appendix~\ref{ch:gmw} which explains the GMW protocol. Readers unfamiliar with ORAM (Trivial ORAM, Circuit ORAM,
Recursive (Circuit) ORAM) may want to read Appendix~\ref{ch:oram} which explains ORAM protocols.

\section{A Taste of \mpc}

\ins{show symphony millionaires and explain in detail}
...
From here on, we instead refer to this language instead as \mpc to more clearly constrast it with \obliv which is presented in
...
\ins{abstractly centralized -- show oblivc millionairs, contrast w/ symphony, mention that simulation + type safety implies no hanging}
...
Here we can see that a lot of the details of the distributed, MPC deployment have leaked into the application code. The
\lstinline[language=c]{main} function is responsible for all sorts of scaffolding such as establishing TCP connections (line 31),
...
a point of comparison, take a look back at Figure~\ref{fig:millionaires}. Notice that this codmakes no mention of TCP connections,
...
\ins{probabilistic -- show 2 ele oram example, touch on goldreich ostrovsky result of lower bound}
...
\ins{high assurance -- refer back to 2 ele oram example, show that security specification is unexpressible in symphony}
