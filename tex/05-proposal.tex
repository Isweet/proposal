...
\mwh{This is not the ``proposal'' -- rather, it is the work still to
  be done. The whole thing is the proposal. So I just deleted that prefix.}
...
\ins{TODO: What should the information flow labels be?} \\

\ins{TODO: What does the type system look like? Sketch it.} \\

\ins{TODO: Don't forget to mention that the syntax doesn't account for
  additional types that will be necessary for handling refinement
  types over principal sets and probability regions.} \\

\ins{TODO: Given a rough candidate for Soundness and PMTO\%, where does the proof strategy need adapting?} \\
\begin{theorem}[Type Soundness]\label{thm:lang-sound}
\end{theorem}
...
\ins{TODO: What does distributed semantics look like? Sketch it.} \\

\ins{TODO: Is the statement of Simulation ``obvious''? If so, explain. If not, why not?} \\

\ins{TODO: Given a rough candidate for Simulation, where does the proof strategy need adapting?} \\
\begin{theorem}[Simulation (\lang)]\label{thm:lang-simulation}
\end{theorem}

\ins{A: The proof is at least complicated by the fact that the sequential semantics is no-longer determinstic.} \\

\ins{IDEA: The mixed semantics of \obliv is more deterministic, shoving distributions into values. Maybe that helps?} \\
